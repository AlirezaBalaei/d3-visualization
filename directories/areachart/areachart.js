...
let graphWidth = d3.select(".graph").attr("width")
let graphHeight = d3.select(".graph").attr("height")
let x = d3.scaleLinear().domain([0,data.length]).range([0,graphWidth])
let y = d3.scaleLinear().domain([data[0],data[data.length-1]]).range([graphHeight,0])
let area = d3.area().x((d,i)=>x(i)).y0(graphHeight).y1(d=>y(d)).curve(d3.curveBasis)
graph.append("path").attr("fill", color[4]).attr("stroke", color[3]).attr("d", area(data))